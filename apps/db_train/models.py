...
class Entry(models.Model):
    ...
    def __str__(self):
        initials = None  # Инициалы
        if self.author.first_name and self.author.middle_name:
            initials = f"{self.author.first_name.upper()[0]}.{self.author.middle_name.upper()[0]}."
        return f"Автор статьи: {self.author.username} - {self.author.last_name} {initials}; Тэг: {[tag.name for tag in self.tags.all()]}"
